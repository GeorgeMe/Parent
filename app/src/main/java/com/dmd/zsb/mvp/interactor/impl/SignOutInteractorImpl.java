 ...
 */
public class SignOutInteractorImpl implements CommonSingleInteractor {

    private BaseSingleLoadedListener<signoutResponse> loadedListener;

    public SignOutInteractorImpl(BaseSingleLoadedListener<signoutResponse> loadedListener) {
        this.loadedListener = loadedListener;
    }

    @Override
    public void getCommonSingleData(JSONObject json) {
        GsonRequest<signoutResponse> gsonRequest=new GsonRequest<signoutResponse>(UriHelper.getInstance().signOut(json),null,new TypeToken<signoutResponse>(){}.getType(), new Response.Listener<signoutResponse>(){
            @Override
